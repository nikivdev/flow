...
pub fn run_task_via_moon(task: &DiscoveredAiTask, project_root: &Path, args: &[String]) -> Result<()> {
...
pub fn build_task_cached(
    ...
    if !status.success() {
        bail!("moon build failed for AI task '{}' with status {}", task.id, status);
    }
...
pub fn run_task_cached(task: &DiscoveredAiTask, project_root: &Path, args: &[String]) -> Result<()> {
...
fn resolve_moon_workspace_and_entry(task: &DiscoveredAiTask, project_root: &Path) -> (PathBuf, PathBuf) {
...
fn compute_cache_key(task: &DiscoveredAiTask, workspace_dir: &Path, run_path: &Path) -> Result<String> {
    let mut hasher = Sha256::new();
    hasher.update(b"flow-ai-task-v1");
    ...
    hash_file_if_exists(&mut hasher, &task.path)?;
    hash_file_if_exists(&mut hasher, &workspace_dir.join("moon.mod.json"))?;
    hash_file_if_exists(&mut hasher, &workspace_dir.join("moon.mod"))?;
    hash_file_if_exists(&mut hasher, &workspace_dir.join("moon.pkg.json"))?;
    hash_file_if_exists(&mut hasher, &workspace_dir.join("moon.pkg"))?;
    if let Ok(out) = Command::new("moon").arg("--version").output() {
...
fn find_built_binary(workspace_dir: &Path) -> Result<PathBuf> {
    let build_dir = workspace_dir.join("_build").join("native").join("release").join("build");
    if !build_dir.exists() {
        bail!("moon build output directory missing: {}", build_dir.display());
    }
